...
export const notifyEndUsersAboutNewBlog = async (users: NewsletterEntry[], blog: BlogEntry): Promise<void> => {
...
export const notifyUsersAboutNewTicket = async (users: User[], ticket: Ticket): Promise<void> => {
	...
	const mailOptions = users.map(({ email, name }) => ({
		...
		html:
			`
				...
				Esperamos que estés muy bien.
				<br/><br/>
				${ticket.companyName} acaba de crear un tiquete buscando los siguientes servicios: ${ticket.service.join(', ')}
				<br/><br/>
