...
export const BlogResolvers = {
  Query: {
    ...
    getMostViewedEntries: async (
      _: any,
      __: any,
      ctx: CustomContext
    ): Promise<BlogEntry[]> => {
      try {
        const latestBlogs = await BlogEntryModel.find({ deleted: false })
          .sort({ views: -1 })
          .limit(4)
          .populate("author", "lastName email name avatar");
        return latestBlogs;
      } catch (err) {
    ...
    getEntriesByAuthor: async (
      _: any,
      { authorId }: Record<string, string>,
      ctx: CustomContext
    ): Promise<BlogEntry[]> => {
      try {
        const BlogEntriesByAuthor = await BlogEntryModel.find({
          author: authorId,
        });
        ...
        if (!BlogEntriesByAuthor) {
          throw await generateErrorObject(
            Errors.NOT_FOUND,
            `There were no valid BlogEntries with for ${authorId}!`,
            ctx
          );
        }
    ...
    getSpecificBlogEntry: async (
      _: any,
      { blogId }: Record<string, string>,
      ctx: CustomContext
    ): Promise<BlogEntry> => {
      try {
        const BlogEntry = await BlogEntryModel.findOne({
          _id: blogId,
        }).populate("author", "lastName email name avatar");
        ...
        if (!BlogEntry) {
          throw await generateErrorObject(
            Errors.NOT_FOUND,
            `There was no valid BlogEntry with id ${blogId}!`,
            ctx
          );
        }
  ...
  Mutation: {
    newBlogEntry: async (
      ...
      if (markdown.trim().length <= 0) {
        throw await generateErrorObject(
          Errors.WRONG_INPUT,
          "You should add at least one paragraph to your Blog Entry",
          ctx
        );
      }
      ...
      try {
        const newBlogEntry = await new BlogEntryModel({
          ...blogInput,
          author: User?.id,
        }).save();
        return newBlogEntry;
    ...
    updateBlogEntry: async (
      ...
      try {
        ...
        const updatedBlogEntry = await BlogEntryModel.findOneAndUpdate(
          { _id },
          blogInput,
          {
            new: true,
    ...
    updateBlogMetrics: async (
      _: any,
      { blogMetricsInput }: BlogMetricsInput,
      ctx: CustomContext
    ): Promise<boolean> => {
      try {
        const { _id, shares, views } = blogMetricsInput;
        BlogEntryModel.findOne(
          { _id },
          async (
            err: CallbackError,
            blogEntry: HydratedDocument<BlogEntry>
          ) => {
            if (err) {
              await generateErrorObject(
                Errors.INTERNAL_SERVER_ERROR,
                err.message,
                ctx
              );
    ...
    deleteBlogEntry: async (
      _: any,
      { blogId }: Record<string, string>,
      ctx: CustomContext
    ): Promise<string> => {
      try {
        const deletedBlogEntry = await BlogEntryModel.findOneAndUpdate(
          { _id: blogId },
          {
            deleted: true,
          },
        ...
        if (!deletedBlogEntry) {
          throw await generateErrorObject(
            Errors.NOT_FOUND,
            "There was no BlogEntry to delete",
            ctx
          );
        }
    ...
    recoverDeletedBlogEntry: async (
      _: any,
      { blogId }: Record<string, string>,
      ctx: CustomContext
    ): Promise<string> => {
      try {
        const recoveredBlogEntry = await BlogEntryModel.findOneAndUpdate(
          { _id: blogId },
          {
            deleted: false,
          },
        ...
        if (!recoveredBlogEntry) {
          throw await generateErrorObject(
            Errors.NOT_FOUND,
            "There was no BlogEntry to recover",
            ctx
          );
        }
