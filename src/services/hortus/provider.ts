...
export class HortusProvider {
	public static async login(email: string, password: string): Promise<HCLoginResponse> {
		try {
	...
	public static async verify(token: string): Promise<HCVerifyResponse> {
	...
	public static async register(input: HCRegisterInput): Promise<HCRegisterResponse> {
		try {
			const client = buildClient()
	...
	public static clearVerifyCache(): void {
		verifyCache.clear()
	}
}
